---
tags: [Nodes]
description: This document lists all available configuration and flags for CryftGo.
sidebar_label: CryftGo Configs + Flags
pagination_label: CryftGo Configs and Flags
sidebar_position: 0
---

# CryftGo Configs and Flags
...
:::tip
[Install Script](/nodes/run/with-installer/installing-cryftgo.md) creates the
node config file at `~/.cryftgo/configs/node.json`. No default file is
created if [CryftGo is built from source](/nodes/run/node-manually.md), you
...
The `--acp-support` flag allows an CryftGo node to indicate support for a
set of [Avalanche Community Proposals](https://github.com/avalanche-foundation/ACPs).

#### `--acp-object` (array of integers)

The `--acp-object` flag allows an CryftGo node to indicate objection for a
set of [Avalanche Community Proposals](https://github.com/avalanche-foundation/ACPs).
...
See [here](/reference/cryftgo/admin-api.md) for more information.

#### `--api-health-enabled` (boolean)

If set to `false`, this node will not expose the Health API. Defaults to `true`. See
[here](/reference/cryftgo/health-api.md) for more information.
...
[here](/reference/cryftgo/index-api.md) for more information.

#### `--api-info-enabled` (boolean)

If set to `false`, this node will not expose the Info API. Defaults to `true`. See
[here](/reference/cryftgo/info-api.md) for more information.

#### `--api-keystore-enabled` (boolean)

If set to `true`, this node will expose the Keystore API. Defaults to `false`.
See [here](/reference/cryftgo/keystore-api.md) for more information.

#### `--api-metrics-enabled` (boolean)

If set to `false`, this node will not expose the Metrics API. Defaults to
`true`. See [here](/reference/cryftgo/metrics-api.md) for more information.
...
categories: network upgrades and optional chain configurations. CryftGo
...
[here](/nodes/configure/chain-configs/chain-config-flags.md). Defaults to `$HOME/.cryftgo/configs/chains`.
If this flag is not provided and the default directory does not exist,
CryftGo will not exit since custom configs are optional. However, if the
flag is set, the specified folder must exist, or CryftGo will exit with an
...
cryftgo --chain-config-content "${chainconfig}"
```

#### `--chain-aliases-file` (string)

Path to JSON file that defines aliases for Blockchain IDs. Defaults to
`~/.cryftgo/configs/chains/aliases.json`. This flag is ignored if
...
Chain specific data directory. Defaults to `$HOME/.cryftgo/chainData`.

## Database

##### `--db-dir` (string, file path)

Specifies the directory to which the database is persisted. Defaults to `"$HOME/.cryftgo/db"`.
...
Specifies the directory in which system logs are kept. Defaults to `"$HOME/.cryftgo/logs"`.
...
CryftGo supports collecting and exporting [OpenTelemetry](https://opentelemetry.io/) traces.
...
`$HOME/.cryftgo/staking/staker.crt`. This flag is ignored if
...
`$HOME/.cryftgo/staking/staker.key`. This flag is ignored if
...
`{subnetID}.json` config file under `--subnet-config-dir`. CryftGo loads
...
Defaults to `$HOME/.cryftgo/configs/subnets`. If the flag is set explicitly,
the specified folder must exist, or CryftGo will exit with an error. This
...
`$HOME/.cryftgo/configs/subnets/p4jUwqZsA2LuSftroCd3zb4ytH8W99oXKuKVZdsty7eQ3rXD6.json`.
...
at this directory. Defaults to the `$HOME/.cryftgo/profiles/`.
...
Sets the directory for [VM plugins](/build/vm/intro.md). The default value is `$HOME/.cryftgo/plugins`.
...
`~/.cryftgo/configs/vms/aliases.json`. This flag is ignored if
