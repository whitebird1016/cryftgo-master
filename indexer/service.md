---
tags: [CryftGo APIs]
description: This page is an overview of the Index API associated with CryftGo.
...
CryftGo can be configured to run with an indexer. That is, it saves (indexes) every container (a
block, vertex or transaction) it accepts on the X-Chain, P-Chain and C-Chain. To run CryftGo
with indexing enabled, set command line flag
[--index-enabled](/nodes/configure/cryftgo-config-flags.md#apis) to true. **CryftGo
...
If `--index-enabled` is changed to `false` from `true`, CryftGo won't start as doing so would
...
This document shows how to query data from CryftGo's Index API. The Index API is only available
        ...
        "outputs": [
          {
            ...
            "output": {
              "addresses": ["X-cryft1wst8jt3z3fm9ce0z6akj3266zmgccdp03hjlaj"],
              "amount": 4999000000,
              "locktime": 0,
              "threshold": 1
            }
          },
          {
            ...
            "fxID": "spdxUxVJQbX85MGxMHbKw1sHxMnSqJ3QBzDyDYEP3h6TLuxqQ",
            "output": {
              "addresses": ["X-cryft1slt2dhfu6a6qezcn5sgtagumq8ag8we75f84sw"],
