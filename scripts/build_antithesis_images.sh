...
# e.g.,
# TEST_SETUP=cryftgo ./scripts/build_antithesis_images.sh                                    # Build local images for cryftgo
# TEST_SETUP=cryftgo NODE_ONLY=1 ./scripts/build_antithesis_images.sh                        # Build only a local node image for cryftgo
...
function build_images {
  ...
  if [[ "${test_setup}" == "xsvm" ]]; then
    # The xsvm node image is built on the cryftgo node image, which is assumed to have already been
    # built. The image name doesn't include the image prefix because it is not intended to be pushed.
    docker_cmd="${docker_cmd} --build-arg CRYFTGO_NODE_IMAGE=antithesis-cryftgo-node:${TAG}"
  fi
  ...
  if [[ -n "${node_only}" ]]; then
    # Skip building the config and workload images. Supports building the cryftgo
  ...
  if [[ "${test_setup}" == "xsvm" ]]; then
    # Ensure cryftgo and xsvm binaries are available to create an initial db state that includes subnets.
    "${AVALANCHE_PATH}"/scripts/build.sh
    "${AVALANCHE_PATH}"/scripts/build_xsvm.sh
    COMPOSE_ENV="${COMPOSE_ENV} CRYFTGO_PATH=${AVALANCHE_PATH}/build/cryftgo CRYFTGO_PLUGIN_DIR=${HOME}/.cryftgo/plugins"
  fi
...
if [[ "${TEST_SETUP}" == "cryftgo" ]]; then
  build_images cryftgo "${AVALANCHE_PATH}/Dockerfile" "${IMAGE_PREFIX}" "${NODE_ONLY:-}"
  ...
  NODE_ONLY=1
  build_images cryftgo "${AVALANCHE_PATH}/Dockerfile" "" "${NODE_ONLY}"
