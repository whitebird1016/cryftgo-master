...
This package implements a simple orchestrator for the cryftgo
...
support of the development and testing of cryftgo and its related
repositories.
...
| flags.go          | FlagsMap    | Simplifies configuration of cryftgo flags  |
...
_ := tmpnet.StartNewNetwork(              // Start the network
    ...
    "/path/to/cryftgo",               // The path to the binary that nodes will execute
    "/path/to/plugins",                   // The path nodes will use for plugin binaries (suggested value ~/.cryftgo/plugins)
)
...
cryftgo on node start. The use of dynamic ports supports testing
...
└── .tmpnet                                              // Root path for the temporary network fixture
    ...
    └── networks                                         // Default parent directory for temporary networks
        └── 20240306-152305.924531                       // The timestamp of creation is the name of a network's directory
            ...
            └── subnets                                  // Directory containing subnet config for both cryftgo and tmpnet
                ...
                └── 2jRbWtaonb2RP8DEM5DBsd7o2o8d...RqNs9 // The ID of a subnet is the name of its configuration dir
                    └── config.json                      // cryftgo configuration for subnet
```

### Common networking configuration

Network configuration such as default flags (e.g. `--log-level=`),
runtime defaults (e.g. cryftgo path) and pre-funded private keys
...
runtime-specific details like the path of the cryftgo binary to
start the node with.
...
The process details of a node are written by cryftgo to
...
distributed system such as cryftgo. The tmpnet fixture enables
