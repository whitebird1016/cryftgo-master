...
func main() {
	...
	startNetworkCmd := &cobra.Command{
		...
		RunE: func(*cobra.Command, []string) error {
			if len(cryftGoPath) == 0 {
				return errCryftGoRequired
			}
	...
	startNetworkCmd.PersistentFlags().StringVar(&cryftGoPath, "cryftgo-path", os.Getenv(tmpnet.CryftGoPathEnvName), "The path to an cryftgo binary")
	startNetworkCmd.PersistentFlags().StringVar(&pluginDir, "plugin-dir", os.ExpandEnv("$HOME/.cryftgo/plugins"), "[optional] the dir containing VM plugins")
