...
// Collects the configuration for running a temporary cryftgo network
type Network struct {
	// Uniquely identifies the temporary network for metrics
	// collection. Distinct from cryftgo's concept of network ID
...
func StartNewNetwork(
	...
	if err := network.EnsureDefaultConfig(w, cryftGoExecPath, pluginDir); err != nil {
		return err
	}
...
func (n *Network) EnsureDefaultConfig(w io.Writer, cryftGoPath string, pluginDir string) error {
	...
	if len(n.DefaultRuntimeConfig.CryftGoPath) == 0 {
		n.DefaultRuntimeConfig.CryftGoPath = cryftGoPath
	}
...
func (n *Network) EnsureNodeConfig(node *Node) error {
	...
	if node.RuntimeConfig == nil {
		node.RuntimeConfig = &NodeRuntimeConfig{
			CryftGoPath: n.DefaultRuntimeConfig.CryftGoPath,
		}
	}
